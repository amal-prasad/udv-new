...
const MAX_SHADE = 0.7; // alpha of the brightest dot, on the night ground
const MAX_DPR = 2;
...
export function Globe({
  ...
  useEffect(() => {
    ...
    let points: Vec3[] = [];
    // Flat x,y pairs, reused across frames: batching dots by shade shouldn't
    // allocate an object per dot per tick. Cleared and refilled each render.
    ...
    function render(elapsed: number) {
      ...
      // culled outright rather than dimmed to near-zero — at this density
      // drawing invisible dots wastes half the frame.
      //
      // Depth and light collapse into one scalar because the dots sit on a
      // near-black ground: dimming the colour and lowering the alpha look
      // identical there, and one scalar buckets where two wouldn't. Per-dot
      // fillStyle assignment is the real cost at ~5k dots, so each bucket
      // pays it once and fills all its rects in a single path.
      ...
      for (const p of points) {
        ...
        const shade = (0.36 + 0.64 * r.z) * (0.45 + 0.55 * intensity);
        const bucket = Math.min(DOT_BUCKETS - 1, Math.max(0, Math.floor(shade * DOT_BUCKETS)));
        buckets[bucket].push(cx + r.x * radius, cy - r.y * radius);
      }
      ...
      for (let b = 0; b < DOT_BUCKETS; b++) {
        ...
        const alpha = ((b + 0.5) / DOT_BUCKETS) * MAX_SHADE;
        g.fillStyle = `rgba(${baseRgb.r}, ${baseRgb.g}, ${baseRgb.b}, ${alpha})`;
        g.beginPath();
        for (let i = 0; i < pts.length; i += 2) {
          g.rect(pts[i] - half, pts[i + 1] - half, DOT_SIZE, DOT_SIZE);
        }
        g.fill();
      }
