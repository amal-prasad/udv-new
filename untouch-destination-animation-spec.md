# Untouch Destination — Animation & Component Spec

This is an addendum to the existing website content script and build prompt. It covers the Skiper UI (Framer Motion) animation layer discussed separately from the scroll-stills plan — read this alongside, not instead of, the content script's sitemap and copy.
...
| Token | Hex | Use |
|---|---|---|
| `ink` | `#1E3140` | headlines, dark section backgrounds (footer, Escape section) |
| `paper` | `#EDEDE7` | light background — cool stone, not cream |
| `pine` | `#3E5C46` | secondary accent, tags — echoes the logo's globe + Himalayan forest |
| `ember` | `#B84B27` | primary CTA buttons only, used sparingly |
| `mist` | `#C7C9C2` | borders/dividers on light backgrounds |
| `cloud` | `#AEB8BE` | muted/secondary text on navy backgrounds |

Principle: real trip photos carry all the color saturation. UI chrome stays restricted to these six tokens so photos never compete with interface color.

### Type

- Headlines / display: **Bricolage Grotesque** — bold, a little irreverent, matches the brand voice ("warm, direct, a little irreverent — like a well-travelled friend").
- Body: **Inter** — plain, readable, sentence case throughout. No tracked-out ALL CAPS labels.

---

## 2. Animation → Section Mapping

| Section (from content script) | Component | Why |
|---|---|---|
| Hero (2.1) | *(none — static photo)* | LCP-critical; don't animate the first paint. |
| **New: Escape banner** (insert after Hero, before Trip Types) | Skiper39 — CrowdCanvas | "Tired of the daily grind" line. Walking silhouettes into the dusk-navy field. |
| Trip Types Strip (2.2) | Skiper34 — StickyCard_003 | Exactly 3 cards (Group / Customised / Experience) = 3 pinned, scaling cards. |
| Featured Itineraries (2.3) | Skiper53 — HoverExpand_002 | Hover/tap-to-expand browsing fits picking a trip better than a static grid. |
| The Stranger Trip Bit (2.4) | Skiper28 — PerspectiveTextScroll | The script says don't bury this differentiator — give it the dramatic 3D reveal, with the real line: "You don't need a travel buddy. You need a departure date." |
| Trust Bar (2.5) | *(none — static)* | |
| Story Teaser (2.6) | *(none on homepage — static/fade)* | Reserved treatment below, on the full Our Story page. |
| Reviews (2.7) | *(none — static)* | Don't animate trust/conversion content. |
| WhatsApp CTA Banner (2.8) | *(none — static)* | |
| Instagram/Gallery Strip (2.9) | Skiper30 — Parallax_002 | Multi-speed columns, real colorful photos get to move here. |
| FAQ Preview / Footer | *(none — static)* | |
| **Our Story** (full page, Section 3) | Skiper19 — stroke path + big reveal, retooled | Line traces a mountain road through the founder narrative beats; ends on the tagline/wordmark instead of the original demo's "skiperui.com" event block (drop that block entirely). |
| Individual itinerary pages (5.2) | Skiper53 — HoverExpand_002 (reused) | Past-trip photo gallery. |

### Technical notes — read before building

1. **One Lenis instance only.** Skiper34 and Skiper28 each wrap themselves in `<ReactLenis root>`. Using both on the same page tree creates two competing smooth-scroll instances (stutter, rubber-banding). Initialize Lenis once at the app root; if any scroll-stills/GSAP sections are still in play elsewhere on the page, wire that single instance to both GSAP's ticker and Framer's scroll hooks — never run two.
2. **`EscapeSection.tsx` needs an asset:** `/images/peeps/all-peeps.png`, the sprite sheet from Skiper39. Source it from wherever the Skiper components came from.
3. **`LoadingScreen.tsx` needs an asset:** `/logo.png`, the Untouch Destination logo exported as a transparent PNG.
4. **Mobile fixes already applied in `EscapeSection.tsx`** (see code below): `dvh` instead of `vh` (iOS URL-bar collapse), crowd count capped below 768px viewports, full crowd reset skipped on height-only resize events, and the whole crowd animation skipped under `prefers-reduced-motion`.
5. Skiper19 and Skiper30 (below) are pasted as originally provided — not yet adapted with the palette/copy above. Skiper34 and Skiper28 likewise. Adapt colors/copy per the mapping table when building each into its target section.

---

## 3. Original Skiper Components (as provided, unmodified)

### Skiper53 — HoverExpand_002
...
### Skiper28 — PerspectiveTextScroll
    ...
    <ReactLenis root>
      ...
      >
        ...
        >
          ...
          >
            {/* Replace with real copy for this section: */}
...
> Note: the placeholder filler text ("Jatt seeweyan cho langheya...") from the original snippet has been removed here — swap in the real Stranger Trip Bit copy per the mapping table above.

---

### Skiper19 — Stroke path following scroll progress
...
const Skiper19 = () => {
  ...
  return (
    ...
    >
      ...
      {/* NOTE: the block below is the original demo's event-promo footer
          ("skiperui.com", punjab/india, ticket prices). This is the block
          to drop for Untouch Destination — replace with the tagline/wordmark
          big-reveal per the Our Story page mapping above. */}
...
### Skiper39 — CrowdCanvas (original, before adaptation)
...
## 4. Built Components (production-ready, for this project)

### `EscapeSection.tsx`

CrowdCanvas adapted with Untouch Destination copy, navy palette, and three mobile fixes: `dvh` instead of `vh`, capped concurrent crowd size below 768px, resize handler ignores height-only mobile URL-bar changes, and the whole animation is skipped under `prefers-reduced-motion`.
...
Logo bob + pulsing-dot loader, `paper` background, auto-dismisses after 1.6s (300ms under `prefers-reduced-motion`).
